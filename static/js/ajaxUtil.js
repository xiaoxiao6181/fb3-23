...
function shouldStartHeartbeat() {
  var pathname = (location.pathname || '').toLowerCase();
  var token = (localStorage.getItem('token') || '').trim();

  // 未登录时不启动心跳，避免公开页面（如注册页）被误重定向
  if (!token) {
    return false;
  }

  // 明确排除登录/注册相关路径，兼容 html 路由与 CF Pages 重写路由
  var publicPathKeywords = ['login', 'reg', 'register', 'signup', 'sign-up'];
  for (var i = 0; i < publicPathKeywords.length; i++) {
    if (pathname.indexOf(publicPathKeywords[i]) > -1) {
      return false;
    }
  }
...
if (shouldStartHeartbeat()) {
  setInterval(function(){
    AjaxUtil.get('/index/login/uplastOptionTime?loadingHide=true').then(function() {
    });
  }, 1000 * 60);
}
